...
def logDist(themass,Amp,meanval,logsd):
    ...
    out=Amp/(themass*np.sqrt(2*np.pi)*logsd)*theexp;
...
#the number of aerosols in each bin
    thenum = np.divide(out, themass)

    end = len(themass)
# now work backwards to check our number distribution
    dlogm = np.subtract(np.log(themass[1:end]), np.log(themass[0:end-1]))
    dD = np.subtract(themass[1:end], themass[0:end-1])
    print 'check total mass:'
    binmass = np.array([x*y for x in out[1:end]for y in dD])
    totmass=np.sum(binmass)
    print totmass
    print 'check total number'
    end = len(thenum)
    binnum = np.array([x**y for x in thenum[1:end] for y in dlogm])
