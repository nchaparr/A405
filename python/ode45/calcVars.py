...
"""
For use in nchap_ode_littlerock
...
output: 
    ...
    dSS = rate of change of Super Saturation 

Functions Defined:
     aero_prms(sigma_w, I_no, M_a, M_w, rho_a, r_a, r, rho_w, TempK):
     returns a and b parameters per W&H ch6 Ex 12
    """
...
def calc_Vars(height, Wt, r0, r_a, Num_a, Tparc, Wvel, rad, SS, rho_a, M_a, I_no, interpTenv, interpTdEnv, interpPress):
    ...
    #for test on total water according to adiabats
    dWsdT=(Ws + Ws**2)*(1.0*c.lv0/(c.Rv*Tparc**2))
    dWsdP = -1.0*(c.eps*es)/(Press - es)**2
    rhovs = es/(c.Rv*Tparc) 
    ...
    if Wt < Ws: #alternate dT equations
